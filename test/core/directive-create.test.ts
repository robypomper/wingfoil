...
describe('CORE_MODULES directive.directiveCreate — P3.1 fit criteria (initialized project, configured identity)', () => {
  ...
  it('AC2: a name colliding with one of the init-scaffolded directives is also a conflict', async () => {
    const result = await directiveCreateFn()({ root: repo, options: { name: 'code-quality' } });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.error.message).toBe('directive already exists: code-quality');
  ...
  it('a missing `--name` is a usage error: exit 2, `missing required argument: --name`', async () => {
    ...
    expect(customDirectiveNames(repo)).toHaveLength(10);
  });
