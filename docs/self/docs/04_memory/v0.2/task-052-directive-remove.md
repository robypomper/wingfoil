...
status: in-progress
rejection_reason: "The implementation is correct and unusually well evidenced: path safety is structural rather than sanitised — the CLI <name> is resolved through loadDirectives/selectDirectivesById and never concatenated into a path, so all fourteen traversal, absolute-path, unicode and case-variant names probed return `unknown directive` with nothing touched; REQ-SEC-07 clause (a) refuses all six task-057 built-ins with the pinned message and keys on the built-in/ directory, not frontmatter (a custom file forged to kind: built-in is still removable, a built-in forged to kind: custom still refused); clause (b) is deterministic, covers global, is reachable on a fresh init, and every one of five mutations to it turns named tests red; the deletion commits exactly one path and leaves a pre-staged unrelated change staged; and every gate and coverage figure in the Execution Notes reproduced verbatim (99 suites / 1506 tests, 98.54/92.27/98.75/99.15, tsc/lint/docs clean with only bug-026). It is returned for three textual corrections it introduced. First, test/core/production-registry.test.ts:41 still announces \"seven operations mutate today\" when the assertion below it now lists eight and never names directive.directiveRemove. Second, src/core/directive-assign.ts:2-4 still says directive remove reuses \"the same two pieces\" after a third was added that it does not reuse — the very doc range dl-062 cites as its evidence. Third, the Execution Notes weak spot 3 claims a shadow removal silently changes the directive in force \"for every role bound to that id\", a case clause (b) makes unreachable, since any id bound by a role or by global refuses removal outright; a future task reading that note would chase a gap that does not exist. While editing those enumerations, bring the two sibling titles current as well (test/core/parity.test.ts:132,149 and test/mcp/read-only-agent-channel.test.ts:82 already omitted memory approve and memory reject on main). Nothing else needs to change, and the four findings — the symlinked-custom/ confinement bug, the [AUTHORING] global refusal wording, P3.3's vacuous \"or workflow step\" precondition, and dl-062's inaccurate Context — are being filed separately."
release: "v0.2"
priority: "Medium"
tags: ["v0.2", "p3"]
ref: "P3.3"
