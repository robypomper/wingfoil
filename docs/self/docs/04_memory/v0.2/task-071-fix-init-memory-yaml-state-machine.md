...
status: in-progress
rejection_reason: "The fix is right and the reading behind it is the one the documents support — REQ-STATE-08 states its rule over the type and names the default machine by value, spec-001 makes `defaults` optional and is silent on the both-absent case, and P1.13 sc.2's Given never places a `defaults` block in the file while its Then names the machine by value, so the engine must own the default and the scenario title is descriptive. The built-in's value is correctly spec-001's worked example rather than REQ-STATE-08's superseded wording, and the built-in is overridable per type and per project (a one-state `sequence` expresses a deliberately lifecycle-less type, verified against the built dist), so nothing is made unconfigurable; before the fix that case was not an opt-out but a hard throw. Mutation testing confirms both arms are pinned — engine arm alone 6 tests red, scaffold arm alone 2 tests red, both 26 red including the end-to-end CLI suite — and every other Execution-Notes claim re-ran true: e2e-smoke unmodified and 17/17, bug-029 untouched, the surviving plain Error unreachable from CLI and MCP, and both inverted expectations pinned the defect rather than a contract. All gates are green and merge-tree is clean against main, task-072 and task-074. Rejected for two narrow reasons. The Execution Notes explain the -0.01pp branch-coverage delta as \"the denominator growing\" when the denominator in fact shrank by one (1195 -> 1194), because a covered branch was deleted along with the dead try/catch; the conclusion that nothing became uncovered is true (92 uncovered on both sides) but the stated mechanism is wrong, and it is wrong inside the audit record rather than in passing. And main has moved to bcc66a9 with task-073's lockfile fix, so dl-035 requires `git merge main` and a gate re-run before approval — which is also what makes a fresh clone of this branch installable, since its gates so far ran against a node_modules symlink and never exercised the stale lockfile. The four findings the notes propose — reconciling REQ-STATE-08's text with spec-001, spec-011's stale memory.yaml row, whether `wingfoil init` should seed the initialising user as approver (today approve and reject are dead in every new project until dna.yaml is hand-edited, with no message pointing at the fix), and the per-type-machine scaffold question — are filed as their own elements rather than left in the notes."
release: "v0.2"
priority: "High"
